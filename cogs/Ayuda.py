...
class Ayuda(commands.Cog):
    ...
    async def comandos(self, ctx: commands.Context):
        ...
        embed.add_field(
            name=f"&elije opc 1|opc 2|...|opc n",
            value="elije entre dos o mas cosas separadas por | defualt: si|no",
        )
        embed.add_field(
            name=f"&caracola_magica <pregunta>",
            value="Preguntale a la caracola  alias: &8b, &caracola",
        )
        embed.add_field(name=f"&tom_gay", value="Tom gay alias: tg, Tg ")
        embed.add_field(
            name=f"&spam @usuario <mensaje>",
            value="Manda entre 15 y 20 mensajes por dm a la persona que lo dirigen",
        )
        embed.add_field(name=f"&desafio <mensaje opcional>", value="desafia al admin")
        embed.add_field(
            name=f"&coca @usuario",
            value="Le da una coca al usuario, si no hay a quien darle la coca no te la puedes quedar!",
        )
        ...
        embed.add_field(
            name="&mason",
            value="Mason, los números! Manda numeritos mágicos que funcionan con g/",
        )
        embed.add_field(
            name="&head_pat @usuario", value="Le da un head pat al usuairo alias: &pet"
        )
        # embed.add_field(name = '&dados `tipo_de_dado` `veces`', value = '')
        embed.set_footer(
            text="Si necesitas más ayuda puedes pedirla con el equipo de desarrollo uwu"
        )
